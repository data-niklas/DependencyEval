...
def main():
    ...
    for item in items:
        ...
        if item["context"] != "":
            code += item["context"] + "\n\n"
        base_code = code + item["function_signature"] + "\n  " + item["function_documentation"] + "\n"
        ...
        try:
            ...
            with open(os.path.join(diff_dir, item["task_name"] + "_vanilla" + ".diff"), "w") as f:
                f.write(diff)
        ...
        try:
            diff = subprocess.run(["diff", "--text", "--unified", "--new-file", "__solution__.py", "__generated__.py"], stderr=subprocess.STDOUT, stdout=subprocess.PIPE).stdout.decode()
            with open(os.path.join(diff_dir, item["task_name"] + "_llm_lsp" + ".diff"), "w") as f:
                f.write(diff)
        except BaseException as e:
            print(e)
            print("Ran into an error while diffing")
